...
//                Log.print(task.getCloudletId() + ","+ indent);
//            }
            //Log.print(indent);
            ...
            if (job.getCloudletStatus() == Cloudlet.SUCCESS) {
                ...
                Log.printLine(
                        //job.getResourceId() +  indent + job.getVmId()
                        ...
                        + "," + job.getAverageSize()
                        + "," + job.getCloudletOutputSize()
                        + "," + job.getCloudletFileSize()
                        + "," + job.getProcessingCost()
                        + ", " + getClassOfJob(job.getProcessingCost())
                    );
    ...
    protected static void printJobList1(List<Job> list) {
        String indent = "    ";
        Log.printLine();        
        Log.printLine("========== OUTPUT ==========");
        Log.printLine("Job ID" + indent + "Task ID" + indent + "STATUS" + indent
        ...
        for (Job job : list) {
            ...
            if (job.getCloudletStatus() == Cloudlet.SUCCESS) {
                ...
                Log.printLine(indent + indent + job.getResourceId() + indent + indent + indent + job.getVmId()
                        ...
                        + indent + indent + dft.format(job.getExecStartTime()) + indent + indent + indent
                        + dft.format(job.getFinishTime()) + indent + indent + indent + job.getDepth()
                        + indent + dft.format(job.getCpuUsage())
                        + indent + dft.format(job.getRamUsage())
                        + indent + dft.format(job.getProcessingCost()));
            } else if (job.getCloudletStatus() == Cloudlet.FAILED) {
                Log.print("FAILED");
                Log.printLine(indent + indent + job.getResourceId() + indent + indent + indent + job.getVmId()
                        + indent + indent + indent + dft.format(job.getActualCPUTime())
    ...
    protected static void printJobList2(List<Job> list) {
        String indent = "    ";
        Log.printLine();        
        Log.printLine("========== OUTPUT ==========");
        Log.printLine("Job ID" + indent + "Task ID" + indent + "STATUS" 
                + indent + "DC ID"
                + indent + "VMID"
                + indent + "Length"
                + indent + "Output Size"
        ...
        for (Job job : list) {
            ...
            if (job.getCloudletStatus() == Cloudlet.SUCCESS) {
                Log.print("SUCCESS, ");
                Log.printLine(job.getResourceId()
                        + ", "+ job.getVmId()
                        + ", "+ dft.format(job.getCloudletLength()) 
                        + ", " + dft.format(job.getCloudletOutputSize())
                        + ", " + dft.format(job.getCloudletTotalLength())
                        + ", " + dft.format(job.getCloudletFileSize())
                        + ", " + dft.format(job.getActualCPUTime())
                        + ", " + dft.format(job.getExecStartTime())
                        + ", " + dft.format(job.getFinishTime()) 
                        + ", " + dft.format(job.getCpuUsage())
                        + ", " + dft.format(job.getRamUsage())
                        + ", " + dft.format(job.getProcessingCost())
                        + ", " + dft.format(getClassOfJob1(job.getProcessingCost())));
            } else if (job.getCloudletStatus() == Cloudlet.FAILED) {
                Log.print("FAILED");
                Log.printLine(indent + indent + job.getResourceId() + indent + indent + indent + job.getVmId()
                        + indent + indent + indent + dft.format(job.getActualCPUTime())
    ...
    private static int getClassOfJob(double cost)
    {
        int res = 0;
        if(cost>0 && cost < 10)
            res = 1;
    ...
    private static int getClassOfJob1(double cost)
    {
        int res = 0;
        if(cost < 5000)
            res = 1;
