...
//New policy
import org.cloudbus.cloudsim.VmAllocationPolicyArima;
...
// New balancer
import org.cloudbus.cloudsim.ex.web.ArimaWebLoadBalancer;
 ...
 * @author Admin
 */
public class ArimaWorkloadExample {
	...
	try (InputStream is = Files.newInputStream(Paths.get("D://customlog2.txt"))) {
	    props.load(is);
	}
                     catch (Exception e) {
	    e.printStackTrace();
	...
	CustomLog.configLogger(props);
                      //CustomLog.redirectToFile("D://customlog2.txt");
	...
	try {
	    ...
	    int refreshTime = 5;
	    WebBroker broker = new WebBroker("Broker", refreshTime,  3600, datacenter0.getId());
	    ...
	    ILoadBalancer balancer = new ArimaWebLoadBalancer(
	...
	try {
	    datacenter = new HddDataCenter(name, characteristics,
		    new VmAllocationPolicyArima(hostList), storageList, 0);
	} catch (Exception e) {
	    e.printStackTrace();
	}
